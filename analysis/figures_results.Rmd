...
p_load("tidyverse", "here", "readr", "lattice", "hexbin", "stargazer", "lme4", 
          ...
          "sjlabelled", "naniar", "summarytools", "GGally", "correlation", "dplyr", "janitor", "ggplot2", 'rio','survey', 'parsedate','sjPlot', 'sjmisc', "scales","reshape2", "skimr", "kableExtra","testthat", "patchwork", "tidyr", "utils", "tidybayes", "ggalt", "gghighlight", "ggridges", "ggpubr", "wesanderson", "viridis", "RColorBrewer", "ggstatsplot")
...
apps <- data.frame(
  Country =c("Germany", "UK", "Switzerland", "Finland", "France", "Italy", "Spain", "Singapore", "Australia", "India"),
...
Plot = ggplot(Germany) +
  ...
  labs(title = 'COVID-19 cases & deaths in Germany', y = '', x = 'Date', size = 14)
...
for (d in 1:nrow(Dates)){
  Plot = Plot +
    # annotate("rect", xmin = Dates$start[d], xmax = Dates$end[d], ymin = 0, ymax = Inf, fill = 'yellow',  alpha = .25) +
    annotate("text", x = Dates$date[d], y = p+10, label = Dates$text[d], angle=90, color = 'black', size=ft, vjust=0+.25, hjust='left', fontface = "bold")
  # geom_segment(x = Dates$date[d], y = 2+Germany$value[(Germany$date == Dates$date[d] & Germany$variable == 'deaths')], yend = p, xend = Dates$date[d], color = 'gray10', size = 1, alpha = 0.5 )
}
Plot
...
```{r, risk perception, fig.width = 18, fig.height= 9}
...
risk_1 <-
  ...
  mutate(Perceived_risk = factor(
    ...
    labels = c(
      ...
      "Concern I will be infected",
      "Concern someone I know will be infected"
    )
  )) %>%
...
fig_risk <- risk %>%
  ggplot(aes(x = Rating, y = perc, fill = wave)) +
  ...
  ylim(0, 60) +
  facet_wrap( ~ Perceived_risk, scales = 'free_x', labeller = label_value) +
  theme_nice() +
  theme(legend.position = "bottom", legend.title = element_blank())
fig_risk

ggsave(
  filename = here("output/fig_risk.pdf"),
  plot = fig_risk,
  dpi = 300,
  units = 'cm',
  height = 16,
...
fig_selfothers <- selfothers %>%
  ...
  scale_fill_manual(values = c("tomato", "orange", "steelblue", "#4cbc7c")) +
  labs(
    title = "Concern for self vs. others",
    x = "",
    y = "% of respondents"
  ) +
  geom_text(
...
acc_track_final <- cbind(acc_track_new, pt)
...
acc_general_final <- cbind(acc_general, pt)


# plot

fig_acc_gen <- acc_general_final %>%
  ggplot(aes(x = measures, y = perc, fill = wave)) +
  geom_col(position = "dodge") +
  scale_fill_manual(values = c("tomato", "orange", "steelblue", "#4cbc7c")) +
  labs(
    title = "General acceptability of six privacy-encroaching measures",
    x = "",
...
ggsave(
  filename = here("output/fig_acc_gen.pdf"),
  plot = fig_acc_gen,
  dpi = 300,
  units = 'cm',
  height = 13,
...
D_sum <- D %>% 
  group_by(rating, value, scenario_type, variable, wave) %>% 
  summarise(n = n()) %>% 
  ungroup
D_sum

app_risk_eff_plot <-
  ggplot(D, aes(x = variable, y = value), trim = FALSE) +
  geom_boxplot(varwidth = TRUE, outlier.shape = NA,  aes(fill = wave)) +
  ...
  labs(title = 'Perception of risk and effectiveness in each scenario and in case of the Corona-Warn-App',
    ...
    facet_wrap( ~ scenario_type,
              ...
              labeller = label_value) +
  theme_nice() +
  theme(legend.position = "bottom", legend.title=element_blank(), axis.text.x = element_text(angle = 0))
app_risk_eff_plot
...
#combine plots
fig_cwa_reasons <- fig_reasons + fig_reasons_not
fig_cwa_reasons
ggsave(
...
mutate(RKI_technology = factor(
    RKI_technology,
    levels = c(1, 2, 3, 4),
    labels = c("Bluetooth", "Location data", "Mobile tower data", "I do not know" )
  )) %>%
  ...
  pivot_longer (names_to = "download", values_to = "perc", cols = Yes:No) %>% 
  mutate(wave = "Wave 3")
...
rki_tech <- rbind(rki_tech3, rki_tech4) %>% 
   mutate(download = factor(
    download,
    levels = c("Yes", "No"),
    labels = c("CWA download: Yes", "CWA download: No")
  ))
...
rki_tech_fig <- rki_tech %>% 
  ...
  theme_nice() +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  labs(title = "What technology does the Corona-Warn-App (CWA) use?", y = "% of respondents", x =
         "") 
rki_tech_fig

ggsave(
  filename = here("output/rki_tech_fig.pdf"),
  plot = rki_tech_fig,
  dpi = 300,
  units = 'cm',
  height = 10,
...
```{r, who has data aceess by download, fig.width = 7, fig.height= 8}


rki_data4  <- data_4 %>% select(RKI_data_access, RKI_download) %>% 
  mutate(whohasaccess = str_split(string = RKI_data_access,
                             pattern = ",")) %>%
  unnest(whohasaccess) %>% 
  drop_na() %>%
  filter(whohasaccess > 0) %>%
  mutate(id = row_number()) %>%
  select(-RKI_data_access)  %>% 
  group_by(whohasaccess, RKI_download) %>% 
  summarise(n = n(), .groups = "keep") %>% 
mutate(whohasaccess = factor(
    whohasaccess,
    levels = c(1, 2, 3, 4, 5, 6, 7),
    labels = c("Employees of the\nresponsible\nhealth authority", "Robert-Koch\nInstitute", "Deutsche\nTelekom", "Nobody", "The public", "Scientists", "I don't know")
  )) %>% 
  mutate(RKI_download = factor(
    RKI_download,
    levels = c(1, 2),
...
rki_data_fig <- rki_data4 %>% 
  ggplot(aes(
    x = reorder(whohasaccess, perc),
    y = perc
  )) +
  geom_col(fill = "#4cbc7c", width = 0.8) +
   geom_text(
    aes(x = whohasaccess, y = perc,
        label = perc),
    size = 3.5,
   vjust = -0.1,
    colour = "black",
   ...
   facet_wrap(~ download, dir = "v", scales = 'free_x', labeller = label_value) +
  ylim(0, 50) +
  theme_nice() +
  theme(legend.position = "bottom") +
  labs(title = "Who has data access?", y = "% of Respondents", x =
         "", caption = "If someone using the CWA tests positive and agrees to share that information with the app, who has access to that data?") 
rki_data_fig

ggsave(
  filename = here("output/rki_data_fig.pdf"),
  plot = rki_data_fig,
  dpi = 300,
  units = 'cm',
  height = 17,
...
cwa_use3 <- data_3 %>% dplyr::select (id, RKI_download, RKI_left_installed, RKI_blutooth, RKI_friends_family, RKI_future_download, RKI_mandatory)%>%
   mutate(RKI_download = factor(
   RKI_download,
    levels = c(1, 2),
    labels = c("Yes", "No") 
  ...
  pivot_longer(names_to = "CoronaWarnApp_Use", 
               values_to = "Answer",
            cols= RKI_download:RKI_mandatory) %>%
  mutate(CoronaWarnApp_Use = factor (CoronaWarnApp_Use,levels = c("RKI_download", "RKI_left_installed", "RKI_blutooth","RKI_friends_family", "RKI_future_download", "RKI_mandatory"),
 )) %>%
...
cwa_use4 <- data_4 %>% dplyr::select (id, RKI_download,RKI_left_installed, RKI_blutooth, RKI_friends_family, RKI_future_download, RKI_mandatory)%>%
   mutate(RKI_download = factor(
   RKI_download,
    levels = c(1, 2),
    labels = c("Yes", "No") 
...
column_spec(3, bold = T)%>%
     column_spec(3, bold = T, color = "white", background = "#71CA97")%>%
   # remove_column(1)%>%
     pack_rows("Have you downloaded the Corona-Warn-App? (all participants)", 1, 2) %>%
    pack_rows("Does the Corona-Warn-App remain installed on your phone? (participants who downloaded the app", 3, 4)%>%
   pack_rows("Do you generally have bluetooth switched on so the the Corona-Warn-App app can operate effectively? (participants who have the App still installed)", 5, 8)%>%
   pack_rows("Have you made any attempts to convince your friends and/or family to download the Corona-Warn-App? (participants who had downloaded the app)", 9, 10)%>%
   pack_rows("Will you download the Corona-Warn-App in the future? (among participants who did not download the app)", 11, 12)%>%
    pack_rows("Do you think that the government should make the Corona-Warn-App mandatory?", 13, 14)%>%
  footnote (general = "", general_title = "", footnote_as_chunk = T, title_format = c("italic"))
table_cwa_use
...
table_cwa_use_tex <- cwa_use %>%
  ...
  kable_paper("striped", full_width = F, position = "left") %>%
   ...
   pack_rows("Do you generally have bluetooth switched on so the the CWA app can operate effectively?", 5, 8)%>%
   pack_rows("Have you made any attempts to convince your friends and/or family to download the CWA?", 9, 10)%>%
   pack_rows("Will you download the CWA in the future?", 11, 12) %>% 
  pack_rows("Do you think that the government should make the CWA mandatory?", 13, 14)
...
cwa_gender3 <- data_3 %>% dplyr::select (id, RKI_download, gender) %>%
  mutate(RKI_download = factor(
    RKI_download,
    levels = c(1, 2),
    labels = c("Yes", "No")
...
cwa_gender4 <- data_4 %>% dplyr::select (id, RKI_download, gender) %>%
  mutate(RKI_download = factor(
    RKI_download,
    levels = c(1, 2),
    labels = c("Yes", "No")
...
cwa_gender_fig <- cwa_gender %>%
  ...
  ylim(0, 100) +
  theme_nice() +
  labs(title = "Corona-Warn-App (CWA download by gender)", y = "% of respondents", x =
         "")+
...
cwa_edu3 <- data_3 %>% dplyr::select (id, RKI_download, education) %>%
  mutate(RKI_download = factor(
    RKI_download,
    levels = c(1, 2),
    labels = c("Yes", "No")
...
cwa_edu4 <- data_4 %>% dplyr::select (id, RKI_download, education) %>%
  mutate(RKI_download = factor(
    RKI_download,
    levels = c(1, 2),
    labels = c("Yes", "No")
...
cwa_edu_fig <- cwa_edu %>%
  ...
  theme_nice() +
  theme(legend.position = "none") +
  labs(title = "CWA download by education", y = "% of respondents", x =
         "")
cwa_edu_fig
...
cwa_age_fig <- cwa_age %>%
  ggplot(aes(x = RKI_download, y = age), trim = FALSE) +
  geom_boxplot(varwidth = TRUE, aes(fill = wave)) +
  scale_fill_manual(name = "Wave",
  ...
  labs(title = 'CWA download by age',
       x = '',
       y = expression('Age')) +
  theme_nice() +
  theme(legend.position = "bottom", legend.title = element_blank())
